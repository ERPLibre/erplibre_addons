...
class TdmController(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        values["tdm_profil_entreprise_count"] = request.env[
            "tdm.profil.entreprise"
        ].search_count([])
        values["tdm_profil_entreprise_stage_count"] = request.env[
            "tdm.profil.entreprise.stage"
        ].search_count([])
        values["tdm_profil_travailleur_count"] = request.env[
            "tdm.profil.travailleur"
        ].search_count([])
        values["tdm_profil_travailleur_stage_count"] = request.env[
            "tdm.profil.travailleur.stage"
        ].search_count([])
        return values
    ...
    # ------------------------------------------------------------
    # My Tdm Profil Entreprise
    # ------------------------------------------------------------
    def _tdm_profil_entreprise_get_page_view_values(
        self, tdm_profil_entreprise, access_token, **kwargs
    ):
        values = {
            "page_name": "tdm_profil_entreprise",
            "tdm_profil_entreprise": tdm_profil_entreprise,
            "user": request.env.user,
        }
        return self._get_page_view_values(
            tdm_profil_entreprise,
            access_token,
            values,
            "my_tdm_profil_entreprises_history",
            False,
            **kwargs,
        )
    ...
    @http.route(
        [
            "/my/tdm_profil_entreprises",
            "/my/tdm_profil_entreprises/page/<int:page>",
        ],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_tdm_profil_entreprises(
        self,
        page=1,
        date_begin=None,
        date_end=None,
        ...
        **kw,
        ...
        values = self._prepare_portal_layout_values()
        TdmProfilEntreprise = request.env["tdm.profil.entreprise"]
        domain = []
        ...
        # tdm_profil_entreprises count
        tdm_profil_entreprise_count = TdmProfilEntreprise.search_count(domain)
        # pager
        pager = portal_pager(
            url="/my/tdm_profil_entreprises",
            url_args={
                "date_begin": date_begin,
                "date_end": date_end,
                "sortby": sortby,
            ...
            total=tdm_profil_entreprise_count,
            page=page,
            step=self._items_per_page,
        )

        # content according to pager and archive selected
        tdm_profil_entreprises = TdmProfilEntreprise.search(
            domain,
            order=order,
            limit=self._items_per_page,
            offset=pager["offset"],
        ...
        request.session[
            "my_tdm_profil_entreprises_history"
        ] = tdm_profil_entreprises.ids[:100]

        values.update(
            {
                "date": date_begin,
                "date_end": date_end,
                "tdm_profil_entreprises": tdm_profil_entreprises,
                "page_name": "tdm_profil_entreprise",
                "archive_groups": archive_groups,
                "default_url": "/my/tdm_profil_entreprises",
                "pager": pager,
                "searchbar_sortings": searchbar_sortings,
                "searchbar_groupby": searchbar_groupby,
                "searchbar_inputs": searchbar_inputs,
        ...
        return request.render("tdm.portal_my_tdm_profil_entreprises", values)

    @http.route(
        ["/my/tdm_profil_entreprise/<int:tdm_profil_entreprise_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def portal_my_tdm_profil_entreprise(
        self, tdm_profil_entreprise_id=None, access_token=None, **kw
    ):
        try:
            tdm_profil_entreprise_sudo = self._document_check_access(
                "tdm.profil.entreprise", tdm_profil_entreprise_id, access_token
            )
        except (AccessError, MissingError):
            return request.redirect("/my")

        values = self._tdm_profil_entreprise_get_page_view_values(
            tdm_profil_entreprise_sudo, access_token, **kw
        )
        return request.render("tdm.portal_my_tdm_profil_entreprise", values)

    # ------------------------------------------------------------
    # My Tdm Profil Entreprise Stage
    # ------------------------------------------------------------
    def _tdm_profil_entreprise_stage_get_page_view_values(
        self, tdm_profil_entreprise_stage, access_token, **kwargs
    ):
        values = {
            "page_name": "tdm_profil_entreprise_stage",
            "tdm_profil_entreprise_stage": tdm_profil_entreprise_stage,
            "user": request.env.user,
        }
        return self._get_page_view_values(
            tdm_profil_entreprise_stage,
            access_token,
            values,
            "my_tdm_profil_entreprise_stages_history",
            False,
            **kwargs,
        )
    ...
    @http.route(
        [
            "/my/tdm_profil_entreprise_stages",
            "/my/tdm_profil_entreprise_stages/page/<int:page>",
        ],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_tdm_profil_entreprise_stages(
        self,
        page=1,
        date_begin=None,
        date_end=None,
        ...
        **kw,
        ...
        values = self._prepare_portal_layout_values()
        TdmProfilEntrepriseStage = request.env["tdm.profil.entreprise.stage"]
        domain = []
        ...
        # archive groups - Default Group By 'create_date'
        archive_groups = self._get_archive_groups(
            "tdm.profil.entreprise.stage", domain
        )
        if date_begin and date_end:
            domain += [
                ("create_date", ">", date_begin),
        ...
        # tdm_profil_entreprise_stages count
        tdm_profil_entreprise_stage_count = (
            TdmProfilEntrepriseStage.search_count(domain)
        )
        # pager
        pager = portal_pager(
            url="/my/tdm_profil_entreprise_stages",
            url_args={
                "date_begin": date_begin,
                "date_end": date_end,
                "sortby": sortby,
            ...
            total=tdm_profil_entreprise_stage_count,
            page=page,
            step=self._items_per_page,
        )

        # content according to pager and archive selected
        tdm_profil_entreprise_stages = TdmProfilEntrepriseStage.search(
            domain,
            order=order,
            limit=self._items_per_page,
            offset=pager["offset"],
        ...
        request.session[
            "my_tdm_profil_entreprise_stages_history"
        ] = tdm_profil_entreprise_stages.ids[:100]

        values.update(
            {
                "date": date_begin,
                "date_end": date_end,
                "tdm_profil_entreprise_stages": tdm_profil_entreprise_stages,
                "page_name": "tdm_profil_entreprise_stage",
                "archive_groups": archive_groups,
                "default_url": "/my/tdm_profil_entreprise_stages",
                "pager": pager,
                "searchbar_sortings": searchbar_sortings,
                "searchbar_groupby": searchbar_groupby,
                "searchbar_inputs": searchbar_inputs,
        ...
        return request.render(
            "tdm.portal_my_tdm_profil_entreprise_stages", values
        )

    @http.route(
        [
            "/my/tdm_profil_entreprise_stage/<int:tdm_profil_entreprise_stage_id>"
        ],
        type="http",
        auth="public",
        website=True,
    )
    def portal_my_tdm_profil_entreprise_stage(
        self, tdm_profil_entreprise_stage_id=None, access_token=None, **kw
    ):
        try:
            tdm_profil_entreprise_stage_sudo = self._document_check_access(
                "tdm.profil.entreprise.stage",
                tdm_profil_entreprise_stage_id,
                access_token,
            )
        except (AccessError, MissingError):
            return request.redirect("/my")

        values = self._tdm_profil_entreprise_stage_get_page_view_values(
            tdm_profil_entreprise_stage_sudo, access_token, **kw
        )
        return request.render(
            "tdm.portal_my_tdm_profil_entreprise_stage", values
        )

    # ------------------------------------------------------------
    # My Tdm Profil Travailleur
    # ------------------------------------------------------------
    def _tdm_profil_travailleur_get_page_view_values(
        self, tdm_profil_travailleur, access_token, **kwargs
    ):
        values = {
            "page_name": "tdm_profil_travailleur",
            "tdm_profil_travailleur": tdm_profil_travailleur,
            "user": request.env.user,
        }
        return self._get_page_view_values(
            tdm_profil_travailleur,
            access_token,
            values,
            "my_tdm_profil_travailleurs_history",
            False,
            **kwargs,
        )
    ...
    @http.route(
        [
            "/my/tdm_profil_travailleurs",
            "/my/tdm_profil_travailleurs/page/<int:page>",
        ],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_tdm_profil_travailleurs(
        self,
        page=1,
        date_begin=None,
        date_end=None,
        ...
        **kw,
        ...
        values = self._prepare_portal_layout_values()
        TdmProfilTravailleur = request.env["tdm.profil.travailleur"]
        domain = []
        ...
        # tdm_profil_travailleurs count
        tdm_profil_travailleur_count = TdmProfilTravailleur.search_count(
            domain
        )
        # pager
        pager = portal_pager(
            url="/my/tdm_profil_travailleurs",
            url_args={
                "date_begin": date_begin,
                "date_end": date_end,
                "sortby": sortby,
            ...
            total=tdm_profil_travailleur_count,
            page=page,
            step=self._items_per_page,
        )

        # content according to pager and archive selected
        tdm_profil_travailleurs = TdmProfilTravailleur.search(
            domain,
            order=order,
            limit=self._items_per_page,
            offset=pager["offset"],
        ...
        request.session[
            "my_tdm_profil_travailleurs_history"
        ] = tdm_profil_travailleurs.ids[:100]

        values.update(
            {
                "date": date_begin,
                "date_end": date_end,
                "tdm_profil_travailleurs": tdm_profil_travailleurs,
                "page_name": "tdm_profil_travailleur",
                "archive_groups": archive_groups,
                "default_url": "/my/tdm_profil_travailleurs",
                "pager": pager,
                "searchbar_sortings": searchbar_sortings,
                "searchbar_groupby": searchbar_groupby,
                "searchbar_inputs": searchbar_inputs,
        ...
        return request.render("tdm.portal_my_tdm_profil_travailleurs", values)

    @http.route(
        ["/my/tdm_profil_travailleur/<int:tdm_profil_travailleur_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def portal_my_tdm_profil_travailleur(
        self, tdm_profil_travailleur_id=None, access_token=None, **kw
    ):
        try:
            tdm_profil_travailleur_sudo = self._document_check_access(
                "tdm.profil.travailleur",
                tdm_profil_travailleur_id,
                access_token,
            )
        except (AccessError, MissingError):
            return request.redirect("/my")

        values = self._tdm_profil_travailleur_get_page_view_values(
            tdm_profil_travailleur_sudo, access_token, **kw
        )
        return request.render("tdm.portal_my_tdm_profil_travailleur", values)

    # ------------------------------------------------------------
    # My Tdm Profil Travailleur Stage
    # ------------------------------------------------------------
    def _tdm_profil_travailleur_stage_get_page_view_values(
        self, tdm_profil_travailleur_stage, access_token, **kwargs
    ):
        values = {
            "page_name": "tdm_profil_travailleur_stage",
            "tdm_profil_travailleur_stage": tdm_profil_travailleur_stage,
            "user": request.env.user,
        }
        return self._get_page_view_values(
            tdm_profil_travailleur_stage,
            access_token,
            values,
            "my_tdm_profil_travailleur_stages_history",
            False,
            **kwargs,
        )
    ...
    @http.route(
        [
            "/my/tdm_profil_travailleur_stages",
            "/my/tdm_profil_travailleur_stages/page/<int:page>",
        ],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_tdm_profil_travailleur_stages(
        self,
        page=1,
        date_begin=None,
        date_end=None,
        ...
        **kw,
        ...
        values = self._prepare_portal_layout_values()
        TdmProfilTravailleurStage = request.env["tdm.profil.travailleur.stage"]
        domain = []
        ...
        # archive groups - Default Group By 'create_date'
        archive_groups = self._get_archive_groups(
            "tdm.profil.travailleur.stage", domain
        )
        if date_begin and date_end:
            domain += [
                ("create_date", ">", date_begin),
        ...
        # tdm_profil_travailleur_stages count
        tdm_profil_travailleur_stage_count = (
            TdmProfilTravailleurStage.search_count(domain)
        )
        # pager
        pager = portal_pager(
            url="/my/tdm_profil_travailleur_stages",
            url_args={
                "date_begin": date_begin,
                "date_end": date_end,
                "sortby": sortby,
            ...
            total=tdm_profil_travailleur_stage_count,
            page=page,
            step=self._items_per_page,
        )

        # content according to pager and archive selected
        tdm_profil_travailleur_stages = TdmProfilTravailleurStage.search(
            domain,
            order=order,
            limit=self._items_per_page,
            offset=pager["offset"],
        ...
        request.session[
            "my_tdm_profil_travailleur_stages_history"
        ] = tdm_profil_travailleur_stages.ids[:100]

        values.update(
            {
                "date": date_begin,
                "date_end": date_end,
                "tdm_profil_travailleur_stages": tdm_profil_travailleur_stages,
                "page_name": "tdm_profil_travailleur_stage",
                "archive_groups": archive_groups,
                "default_url": "/my/tdm_profil_travailleur_stages",
                "pager": pager,
                "searchbar_sortings": searchbar_sortings,
                "searchbar_groupby": searchbar_groupby,
                "searchbar_inputs": searchbar_inputs,
        ...
        return request.render(
            "tdm.portal_my_tdm_profil_travailleur_stages", values
        )

    @http.route(
        [
            "/my/tdm_profil_travailleur_stage/<int:tdm_profil_travailleur_stage_id>"
        ],
        type="http",
        auth="public",
        website=True,
    )
    def portal_my_tdm_profil_travailleur_stage(
        self, tdm_profil_travailleur_stage_id=None, access_token=None, **kw
    ):
        try:
            tdm_profil_travailleur_stage_sudo = self._document_check_access(
                "tdm.profil.travailleur.stage",
                tdm_profil_travailleur_stage_id,
                access_token,
            )
        except (AccessError, MissingError):
            return request.redirect("/my")

        values = self._tdm_profil_travailleur_stage_get_page_view_values(
            tdm_profil_travailleur_stage_sudo, access_token, **kw
        )
        return request.render(
            "tdm.portal_my_tdm_profil_travailleur_stage", values
        )
