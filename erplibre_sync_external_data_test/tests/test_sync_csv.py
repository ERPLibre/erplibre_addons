...
class TestCsvSingleLine(SyncTestBase):
    ...
    def test_transform_sale_order_has_two_lines(self):
        se = self._make_sync_exec(self.sync_model, self.file_b64)
        se.action_process_sync_data()
        se.action_process_transform({})
        entry = self._entries_from(se)
        self.assertEqual(len(entry.sale_order_id.order_line), 2)

    def test_transform_sale_order_line_products(self):
        se = self._make_sync_exec(self.sync_model, self.file_b64)
        se.action_process_sync_data()
        se.action_process_transform({})
        entry = self._entries_from(se)
        product_names = entry.sale_order_id.order_line.mapped("product_id.name")
        self.assertIn("Services conseil", product_names)
        self.assertIn("Formation", product_names)
...
class TestCsvMultiLine(SyncTestBase):
    ...
    def test_transform_each_order_has_two_lines(self):
        se = self._make_sync_exec(self.sync_model, self.file_b64)
        se.action_process_sync_data()
        se.action_process_transform({})
        for entry in self._entries_from(se):
            self.assertEqual(
                len(entry.sale_order_id.order_line),
                2,
                f"{entry.project_code}: expected 2 order lines",
            )

    def test_transform_products_created_once(self):
        """Products are shared across orders — not duplicated per entry."""
        se = self._make_sync_exec(self.sync_model, self.file_b64)
        se.action_process_sync_data()
        se.action_process_transform({})
        count = self.env["product.product"].search_count(
            [("name", "in", ["Services conseil", "Formation"])]
        )
