...
class SurveyPortalView(http.Controller):
    ...
    @http.route("/my/survey/ans", type="http", auth="public", website=True)
    def portal_my_survey(self):
        """Controller to send survey data to portal"""
        survey = (
            request.env["survey.survey"]
            .sudo()
            .search([("user_id", "=", request.uid)])
        )
        values = {
            "survey_list": [
                {
        ...
        return request.render(
            """survey_matrix_selection.portal_survey_result""", values
        )

    @http.route(
        "/my/survey/<int:survey_id>", type="http", auth="public", website=True
    )
    def survey_view(self, survey_id):
        """Controller function to send specific survey data"""
        question_list = []
        survey_questions_line = (
        ...
        return request.render(
            "survey_matrix_selection.portal_survey_result_view", values
        )
...
class SurveyLoadContent(http.Controller):
    ...
    def load_states(self, **kwargs):
        ...
        state_ids = (
            ...
            .sudo()
            .search([("country_id.name", "=", kwargs["country_id"])])
        )
        return {
